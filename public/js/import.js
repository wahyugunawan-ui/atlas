 ...
 */
import { fetchImports, fetchPeriods, fetchUnmatched, uploadImport } from './api.js';
...
export async function refreshImportTab() {
  try {
    ...
    $('imp-periods').innerHTML = periods.length ? periods.map((p) =>
      ...
      `<button onclick="reimportPeriod('${esc(p.period)}')" class="mt-2 text-[11px] font-bold text-slate-500 hover:text-slate-800">` +
      `<i class="ph ph-arrow-clockwise"></i> impor ulang bulan ini</button></div>`).join('')
