> # DOKUMEN SEJARAH — JANGAN DIPAKAI MERANCANG
>
> Ini rencana migrasi tertanggal **12 Agustus 2026**, disimpan apa adanya. Dibiarkan
> hidup karena satu bagiannya masih satu-satunya sumber tertulis: alasan "laptop dulu,
> VPS menyusul" dan analisis akses-bersamaan. Itu saja yang masih berlaku.
>
> **Yang di bawah ini sudah TIDAK benar:**
>
> | Ditulis di sini | Sebenarnya sekarang |
> |---|---|
> | SQLite lewat `node:sqlite`, dua berkas `.db` | PostgreSQL 17 + PostGIS, database `astra` dan `astra_customers` |
> | Folder `logika/` `publik/` `tes/` `skrip/` | `backend/core` `backend/server` `frontend/` `test/` `scripts/` |
> | Tabel `agregat` `pos` `impor` `konsumen` | `sales` `outlets` `imports` `customers` |
> | Kolom `kode_kel` `kode_pos` `jumlah` | `village_code` `outlet_code` `quantity` |
> | Nama Indonesia untuk kode | Nama Inggris — lihat aturan penamaan di `CLAUDE.md` |
> | Rute `/masuk` `/api/agregat` `/api/pos` | `/login` `/api/summary` `/api/outlets` |
> | Jangkauan diperkirakan dengan menyebar titik | Dihitung eksak PostGIS lewat `ST_Intersection` |
> | Poligon disiapkan geopandas + GDAL | Node membaca berkas sumber langsung; geopandas tidak dipakai |
>
> **Untuk tahu apa yang harus dibangun sekarang, baca [PRD.md](../PRD.md).**
>
> Isinya sengaja tidak disunting. Dokumen sejarah yang dirapikan berhenti jadi dokumen
> sejarah, dan tabel di atas jadi tidak ada artinya.
